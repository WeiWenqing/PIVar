...
#use Math::Trig;
#############################################
my ($in,$out,$p,$c,$e,$m,$r,$s,$win_max,$Help);
GetOptions
(
	"i=s" => \$in,
	"p=s" => \$p,
...
foreach my $file (@files){
	...
	while(<$fh>){
		chomp;
		my @tmp=split /\t/;#chr10   35727252        35727252        chr10   35727252        35727252
...
foreach my $file (@files){
        ...
        while(<$fh>){
                chomp;
                my @tmp=split /\t/;#C313T   chr12   131590314       131590314       NOID4046        0.5872 (-5..-1)
                my $id=$tmp[-5]."_".$tmp[-4]."_".$tmp[-3];
		if ($tmp[-1]<0.1){
	                $pval{$id}=1;
		}
        }
